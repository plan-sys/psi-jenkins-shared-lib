def call(taskFamily, cluster, service, dockerImageName, releaseNumber){
  sh """
      ...
      TASK_WITH_NEW_VERSION_NUM=\$(echo \$OLD_TASK_DEF | jq -r --arg RELEASE_NUMBER \$RELEASE_NUMBER '.taskDefinition.containerDefinitions[0].environment |= map(if .name=="VERSION" then .value=\$RELEASE_NUMBER else . end)'
